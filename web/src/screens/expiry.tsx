...
// v0.5.2 ADR-018: when `embedded` is true, this screen is mounted as a
// tab inside /alerts and skips its own header / nav chrome (the parent
// /alerts shell provides them).
export function ExpiryScreen({ embedded = false }: { embedded?: boolean } = {}): JSX.Element {
  ...
  // Embedded inside /alerts? Skip the page chrome (header + ScreenLayout).
  // The parent /alerts shell provides the back button + tabs.
  const Body = (
    <main
      data-testid="expiry-screen"
      className="flex flex-1 flex-col gap-3 overflow-y-auto px-4 py-4"
    >
      <div data-testid="expiry-filter" className="border-hair flex rounded-xl border bg-white p-1">
        {filterKeys.map((k) => (
          <button
            key={k}
            type="button"
